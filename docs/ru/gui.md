...
Полный список доступен в меню «☰» → «Сочетания клавиш».

## Внешний вид

Интерфейс следует GNOME HIG и использует системную тему: светлая и тёмная
приходят из настроек рабочего стола, отдельного переключателя в приложении нет.

Окно адаптивное. При ширине меньше 550 точек переключатель вкладок переезжает
из шапки вниз, поэтому приложением удобно пользоваться и в узком окне.
