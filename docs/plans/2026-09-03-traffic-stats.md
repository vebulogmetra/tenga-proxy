...
Выполнено 2026-09-03 целиком, четырьмя коммитами: `f3c64f7` (policy),
`b9f37ad` (чтение статистики), `e97db3f` (тег профиля), `648e077` (таймер).

**Проверка на живом ядре.** Полноценный запуск интерфейса с подключением, как
предполагал шаг 1 задачи 4, оказался невозможен: адрес Stats API зашит
константой `DEFAULT_STATS_API_ADDR` (`src/core/config.py:93`), порт 10085 занят
рабочим ядром владельца, и второй экземпляр конфликтовал бы с ним — это прямо
запрещено ограничениями. Связка проверена иначе, на том же уровне полноты:
настоящий `XrayManager` поднял собственное ядро на порту 10087 с конфигурацией
профиля, где тег outbound — имя `🇵🇱Польша | XHTTP VPN#6`, а рядом служебный
`direct`. Секции `stats`, `api` и `policy` в конфигурацию не вписывались
намеренно, чтобы их добавил сам код.

Результат: `_inject_stats_api` добавила
`{"system": {"statsOutboundUplink": true, "statsOutboundDownlink": true}}`;
`get_traffic()` до трафика вернула `0/0`, после трёх запросов через socks —
`upload=240, download=2453`; счётчик найден по тегу-имени профиля, служебный
`direct` в счёт не пошёл. То есть все три звена работают вместе на настоящем
ядре, а не только под подменой в тестах.

**Отступления от плана.**

1. Файла `tests/test_core_xray_manager.py` не существовало — план считал его
   готовым. Создан; `_inject_stats_api` и `get_traffic` не были покрыты вовсе.
2. Конструктор `XrayManager` запускает бинарник ради версии, поэтому во всех
   тестах `_fetch_version` подавляется — как в соседнем `test_core_latency_probe`.
3. Подменяется атрибут `subprocess.run` у модуля, а не строка `"subprocess.run"`:
   `xray_manager` импортирует модуль целиком, и так подмена гарантированно видна.
4. Служебных тегов оказалось больше, чем предполагал план. Помимо `direct`,
   `vpn` и `api` `config_builder` заводит три канала DNS — `main-dns`,
   `local-dns`, `vpn-dns`; без них резолвинг попадал бы в счёт трафика прокси.
   Зато `block` и `dns-out` из плана не существуют, и в список они не вошли.
5. Поле `connection_statistics` удалено, а не превращено в выключатель: оно не
   выведено в настройки и не читалось ни одной строкой кода, то есть управлять
   им было невозможно, а значение по умолчанию `False` молча прятало бы работу.
   Существующие `settings.json` с этим ключом читаются по-прежнему: `from_dict`
   перебирает поля класса, а не ключи файла.
6. Существующий `test_connection_monitor_start_enabled` требовал ровно одного
   `timeout_add`; таймеров теперь два, и проверка уточнена под новое устройство.
   Добавлены два теста на снятие таймера трафика, в том числе при раннем
   возврате из `stop()` — требование плана без покрытия.
7. Добавлены тесты сверх плана: на состав команды опроса, на таймаут вызова, на
   ненулевой код возврата, на имя счётчика не из четырёх частей и на то, что
   безымянный профиль с тегом `proxy` по-прежнему учитывается.

**Не сделано.** Счётчик не проверен в самом окне приложения: для этого нужно
подключение, а рабочее трогать нельзя. Проверены слой ядра и слой опроса;
`metrics_text()` (`src/ui/logic/status.py`) уже покрыта тестами и печатает то,
что ей передали.

Тесты: 492 проходят, 197 отобраны по маркеру `gtk`. GTK-тесты в этой системе
падают, пока имя `ru.tenga.Proxy` занято рабочим приложением, — проверено, что
те же 35 падений воспроизводятся и без правок этой задачи.
