...
Этап выполнен 2026-09-02. Ветка `feature/ui-adwaita`, 10 коммитов поверх
результатов этапа 3 (`85ff898`).

| Коммит | Содержание |
|--------|------------|
| `7c9de33` | план этапа |
| `3c70733` | дерево пунктов меню и его свойства |
| `12d365c` | сериализация в варианты dbusmenu |
| `4815b2a` | переименование пакета в `tray4`, фикстуры приватной шины |
| `346ab8d` | StatusNotifierItem поверх D-Bus |
| `b465209` | три различимые иконки (B17) |
| `ba4a92f` | построение меню из состояния |
| `fa1c681` | TrayController |
| `be05b67` | действия `connect-profile(i)` и `activate-window` |
| `c375b2c` | трей в приложении, `--no-tray` для GTK4 |
| `7ccf8d5` | починка плавающего теста брейкпоинта |

### Отклонения от плана

1. **Пакет назван `tray4`, а не `tray`.** Пакет `src/ui/tray/` перекрывает
   модуль `src/ui/tray.py` — Python предпочитает пакет, и GTK3-интерфейс
   переставал импортироваться (`ImportError: cannot import name 'TrayIcon'`).
   План исходил из структуры дизайн-документа, где GTK3-кода уже нет, но его
   удаление — этап 5. Переименование в `tray` откладывается туда же, по образцу
   `dialogs4`.

2. **`GetGroupProperties` тоже потребовал `VariantBuilder`.** Находка 3 плана
   касалась только рекурсивного `av`, но `GLib.Variant("(ia{sv})", …)` падает с
   `TypeError: Expected GLib.Variant, but got bool` по той же причине: оверрайд
   PyGObject не собирает кортеж со словарём готовых вариантов. Добавлен
   `_pair_variant`.

3. **Идентификаторы в меню сквозные, а не по уровням.** План ожидал у детей
   корня `[1, 2]`; при вложенности нумерация продолжается вглубь, поэтому после
   пункта с двумя потомками следующий брат получает 4. Ожидания в тестах
   исправлены, поведение оставлено — оно соответствует спецификации.

4. **`ToolTip` — четвёрка `(icon, pixmaps, title, description)`.** Текст
   подсказки лежит в индексе 3, а не 2, как было записано в плане.

5. **Иконка «подключение» переделана после осмотра.** Первый вариант —
   незамкнутая дуга — читался как случайный обрыв. Заменён на пунктирное кольцо
   с точкой в центре: на 22 px однозначно отличается от двух других и на
   светлой, и на тёмной панели.

6. **`activate_action` переопределён с сохранением совместимости.** План
   предлагал принимать только целое; существующие тесты этапа 3 передают
   `GLib.Variant` и `None`, поэтому метод принимает все три формы.

7. **Заглушки вынесены в `tests/support/tray.py`.** План допускал копию в двух
   файлах; общий модуль надёжнее — копии разошлись бы при первой правке.

8. **Плавающий тест починен вне рамок этапа.** `test_narrow_window_moves_the_
   switcher_down` падал примерно в трети запусков — проверено, что и на коммите
   этапа 3 тоже, то есть это не регрессия трея. Цикл ожидания выходил по
   «очередь пуста» раньше, чем композитор выделял окну размер. Заменён на
   ожидание самого брейкпоинта с таймаутом.

9. **Скриншот панели снять не удалось.** `import -window root` не работает под
   XWayland, а `org.gnome.Shell.Screenshot.ScreenshotArea` запрещён политикой
   (`AccessDenied`). Вместо снимка проверено, что все три имени иконок
   разрешаются в файлы через `Gtk.IconTheme` с тем же `IconThemePath`, который
   отдаётся панели, — то есть тем же механизмом, каким пользуется сама панель.

### Живая проверка

Тестовое приложение запускалось на копии конфигурации пользователя (133 записи:
130 профилей и 3 группы) с `TENGA_CONFIG_DIR`, приложение пользователя не
останавливалось.

Подтверждено на живой шине сессии:

- элемент `org.kde.StatusNotifierItem-<pid>-1` зарегистрирован у настоящего
  `org.kde.StatusNotifierWatcher` **рядом** с элементом приложения пользователя;
- свойства: `Category=ApplicationStatus`, `IconName=tenga-proxy-disconnected`,
  `IconThemePath` — каталог с иконками, `Menu=/MenuBar`, `ItemIsMenu=false`,
  `ToolTip` содержит «Tenga Proxy: отключено»;
- `com.canonical.dbusmenu`: `Version=3`, `TextDirection=ltr`, `Status=normal`;
- `GetLayout` отдаёт верхний уровень из 7 пунктов и 4 разделителей ровно в
  спроектированном порядке;
- подменю профилей: 20 пунктов из 133 плюс «Показать все…», у каждого
  `toggle-type=radio`;
- клик по «Настройки…» открыл диалог настроек с реальными значениями конфига
  (`xray0`, MTU 1450, порт 2080);
- клик по «Добавить профиль…» открыл диалог добавления;
- после остановки приложения элемент исчез с шины, элемент пользователя остался.

### Найденный дефект (не чинился)

**Диалоги открываются поверх уже открытых.** Клик по «Добавить профиль…» при
открытом диалоге настроек показал второй диалог поверх первого. Все
`_open_*` в `src/ui/application.py` вызывают `present(self._window)` без
проверки, открыт ли другой диалог. Дефект общий для приложения (наследие
этапа 3), воспроизводится и через окно, к трею отношения не имеет. Правка
затрагивает весь набор диалогов — вынесена в этап 5.

### Происшествие

При проверке меню был отправлен `Event("clicked")` на пункт с идентификатором 8
в расчёте, что это «Открыть окно». Нумерация сквозная, и 8 оказался профилем в
подменю — то есть была предпринята попытка подключения, которую пользователь
проверять запретил. Последствий нет: второе ядро не запустилось,
`Failed to start: device or resource busy` (порт занят приложением
пользователя), соединение пользователя не прерывалось. Проверено сразу после:
xray пользователя (PID 148729) работал, `xray0` и `tun0` подняты. Дальнейшие
клики выполнялись только по идентификаторам, сверенным через
`GetGroupProperties`. Побочно это подтвердило всю цепочку целиком: пункт меню →
`Event` → `app.connect-profile(id)` → `ConnectionService` → неудача → трей
перешёл в состояние ошибки и вернул иконку «отключено».

### Метрики

| Показатель | Значение |
|------------|----------|
| Файлов изменено/создано | 22 |
| Строк добавлено | +2488 |
| Строк удалено | −11 |
| Тестов без дисплея | 502 (было 375) |
| Тестов GTK | 110 (было 96) |
| Новых тестов этапа | 117 |

Прогон: `uv run pytest -q` — 502 passed, 187 deselected;
`make test-gtk` — 110 passed (четыре прогона подряд без падений);
`ruff check` и `ruff format --check` по файлам этапа — чисто.

### Не сделано

- **Уведомления.** GTK3-трей умеет `Notify.Notification`; в GTK4 это
  `Gio.Notification` через `Gio.Application.send_notification`. К трею отношения
  не имеет, вынесено за рамки этапа.
- **Проверка переживания перезапуска панели.** Код на это рассчитан
  (`bus_watch_name` вместо однократной регистрации) и покрыт тестом
  `test_the_item_registers_when_the_watcher_appears_later`, но вживую
  не проверялось: перезапуск расширения AppIndicator затронул бы иконку
  работающего приложения пользователя.
- **Проверка в KDE.** Критерий приёмки из общего плана упоминает KDE; среда
  разработки — GNOME, другой среды нет.
