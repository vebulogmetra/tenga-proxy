...
class ProfileVpnSettingsDialog(Gtk.Dialog):
    ...
    def __init__(self, profile: ProfileEntry, parent: Gtk.Window | None = None):
        ...
        # Bean из отредактированной ссылки, ожидающий сохранения диалога.
        self._pending_bean: ProxyBean | None = None
        self._last_link_had_remark = False
    ...
    def _create_profile_page(self) -> Gtk.Widget:
        ...
        box.pack_start(edit_frame, False, False, 0)
        ...
        self._edit_link_entry.set_text(self._profile.bean.to_share_link())
    ...
    def _on_apply_link_clicked(self, button: Gtk.Button) -> None:
        ...
        if self._last_link_had_remark:
            self._profile_name_entry.set_text(bean.display_name)

        self._edit_status_label.set_markup(f"<span color='green'>{message}</span>")

    def _commit_pending_bean(self) -> None:
        """Перенести разобранный bean в профиль.

        Вызывается только из `save_settings()` — при Отмене профиль остаётся
        с исходным bean.
        """
    ...
    def _apply_edited_link(self, link: str) -> tuple[bool, str]:
        ...
        Keeps the profile identity (id, group, vpn/routing settings) and only
        swaps the bean. Returns (success, status message). On failure the
        staged bean is left untouched.

        Разобранный bean держится в `_pending_bean` и попадает в профиль лишь
        в `save_settings()`: `get_profile()` отдаёт живой объект хранилища, и
        подмена прямо здесь пережила бы Отмену — любой последующий
        `profiles.save()` записал бы её на диск.
        """
        ...
        self._last_link_had_remark = "#" in link and bool(link.split("#", 1)[1])
